...
{{-- page title --}}
@section('page_title', 'Edit Arm Details')
@section('content')

    <div class="container-fluid py-2">
        <div class="mb-4">
            <h2 class="fs-4 fw-bold text-dark mb-1">{{ "Edit Arm Details" }}</h2>
        ...
        <div class="card border border-light shadow-sm rounded-4 overflow-hidden">
            ...
            <div class="card-body p-3 p-md-4">
                <form action="{{ route('admin.arms.update', $arm->id) }}" method="post">
                    @csrf
                    @method('PUT')
                    <div class="mb-3 row">
                        <div class="col-12">
                            <div class="input-group">
                                <label for="class_name" class="input-group-text col-form-label">Arm Name</label>

                                <input type="text" name="name" id="name"
                                    class="form-control @error('name') is-invalid @enderror"
                                    value="{{ old('name', $arm->name) }}" placeholder="Enter arm name" required>
                        ...
                        <div class="col-12">
                            <div class="input-group">
                                ...
                                <select name="is_enabled" id="is_enabled"
                                    ...
                                    <option value="1" {{ old('is_enabled', $arm->is_enabled) == 1 ? 'selected' : '' }}>Yes</option>
                                    <option value="0" {{ old('is_enabled', $arm->is_enabled) == 0 ? 'selected' : '' }}>No</option>
                                </select>
                                {{-- error message --}}
                                @error('is_enabled')
                                    <span class="p-3">
                    ...
                    <div class="d-grid d-md-block mt-3">
                        <button type="submit" class="btn btn-primary"><i class="fa-solid fa-save"></i> Update Arm</button>
                    </div>
                </form>
            </div>
